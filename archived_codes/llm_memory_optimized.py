...
#from silero_optimized import VoiceTranscriber
...
VAD_PATH = r"D:\codes\projects\schpj\4thyr\txt_files\silero_vad.jit"
LOG_PATH = Path("response.txt")
WAIT_FOR_SPEECH_BEFORE_NEXT_PROMPT = True
...
model = AutoModelForSeq2SeqLM.from_pretrained(TRANSLATER_MODEL_PATH).to(TRANSLATION_DEVICE).eval()

tts_model = VitsModel.from_pretrained(TTS_MODEL_PATH).to(TTS_DEVICE).eval()
...
def translate(text, src_lang, target_lang):
    ...
    inputs = tokenizer(text, return_tensors="pt", truncation=True, max_length=256).to(TRANSLATION_DEVICE)
...
def generate_tts(text_chunk):
    inputs = tts_tokenizer(text_chunk, return_tensors="pt").to(TTS_DEVICE)

    #with torch.inference_mode():
    with torch.no_grad():
        output = tts_model(**inputs).waveform

    output_numpy = output.squeeze().detach().cpu().numpy()
    ...
    while True:
        ...
        try:
            ...
            with LOG_PATH.open('a', encoding='UTF-8') as f:
                f.write(f"{burmese_sentence},{translate_time:.4f},{tts_time:.4f}\n")

            audio_queue.put(wav)
        except Exception as exc:
            print(Fore.RED + f"\nSpeech worker error: {type(exc).__name__}: {exc}")
        finally:
            speech_queue.task_done()
...
def stream_response(prompt):
    ...
    print('\n')
    if WAIT_FOR_SPEECH_BEFORE_NEXT_PROMPT:
        speech_queue.join()
        audio_queue.join()
...
def ensure_vector_db():
    if not vector_db_ready:
        create_vector_db(fetch_conversations())
